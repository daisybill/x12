  ...
  describe "#initialize" do
    ...
    context "a loop" do
      ...
      context "containing another loop" do
        ...
        it "parses the outer loop" do
          expect(subject[X12::Structures::Loop].keys).to eq [ "outer" ]
        end

        it "parses the inner loop" do
          inner = subject[X12::Structures::Loop]["outer"].nodes
          expect(inner.first).to be_an_instance_of X12::Structures::Loop
        end
      end
      ...
      context "containing a segment" do
        ...
        it "parses the inner loop" do
          inner = subject[X12::Structures::Loop]["outer"].nodes
          expect(inner.first.class).to eq X12::Structures::Segment
        end
      end
    ...
    context "a segment" do
      ...
      it "parses the segment" do
        expect(subject[X12::Structures::Segment].keys).to eq [ "ST" ]
      end

      it "parses the fields" do
        segment = subject[X12::Structures::Segment]["ST"]

        expect(segment.nodes.map(&:class)).to eq [ X12::Structures::Field ]
      end
    end
    ...
    context "a composite" do
      ...
      it "parses the composite" do
        expect(subject[X12::Structures::Composite].keys).to eq [ "C043" ]
      end

      it "parses the fields" do
        composite = subject[X12::Structures::Composite]["C043"]

        expect(composite.nodes.map(&:class)).to eq [ X12::Structures::Field ]
      end
    end
  end
end
