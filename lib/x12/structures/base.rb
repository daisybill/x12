    ...
    class Base
      ...
      def show(ind = '')
        ...
        self.to_a.each{|i|
          ...
          if i.kind_of?(X12::Structures::Segment) && i.nodes[0]
            i.find_field(i.nodes[0].name)
          end
          i.nodes.each{|j|
            case
            when j.kind_of?(X12::Structures::Base)
              j.show(ind+'  ')
            when j.kind_of?(X12::Structures::Field)
      ...
      def find(e)
        case self
          ...
          # Depth now
          nodes.each do |i|
            res = i.find(e) if i.kind_of?(X12::Structures::Loop)
